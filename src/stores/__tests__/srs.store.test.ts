...
describe('srs store', () => {
  ...
  it('registerCard is idempotent and creates fresh state', () => {
    const s = useSRSStore()
    s.registerCard('vue', 'fc-1')
    const first = s.getCard('vue', 'fc-1')
    expect(first?.repetitions).toBe(0)
    expect(first?.nextReview).toBeNull()

    s.registerCard('vue', 'fc-1') // no-op
    expect(s.getCard('vue', 'fc-1')).toEqual(first)
  })

  it('review applies SM-2 and updates nextReview', () => {
    const s = useSRSStore()
    s.registerCard('vue', 'fc-1')
    const result = s.review('vue', 'fc-1', 5)
  ...
  it('cards are namespaced per course', () => {
    ...
    expect(s.getCard('vue', 'fc-1')).not.toBeNull()
  ...
  it('dueCount counts new cards (nextReview null) as due', () => {
    const s = useSRSStore()
    expect(s.dueCount('vue')).toBe(0)
    s.registerCard('vue', 'a')
    s.registerCard('vue', 'b')
    expect(s.dueCount('vue')).toBe(2)
    s.review('vue', 'a', 5) // schedules into the future
    expect(s.dueCount('vue')).toBe(1)
  })

  it('persists across reinstantiation', () => {
    const a = useSRSStore()
    a.registerCard('vue', 'fc-1')
    setActivePinia(createPinia())
    const b = useSRSStore()
    expect(b.getCard('vue', 'fc-1')).not.toBeNull()
  })
  ...
  it('reset clears everything and removes storage key', () => {
    ...
    s.registerCard('vue', 'fc-1')
    s.reset()
    expect(s.getCard('vue', 'fc-1')).toBeNull()
