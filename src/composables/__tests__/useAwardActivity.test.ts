...
describe('useAwardActivity', () => {
  ...
  it('completeLesson (first time) triggers all 4 side effects', () => {
    ...
    const result = award.completeLesson('stub', 'stub/intro/hello-world')

    expect(result.awarded).toBe(true)
    expect(cp.isLessonComplete('stub', 'stub/intro/hello-world')).toBe(true)
    expect(xp.xp).toBe(XP_REWARDS.LESSON_COMPLETE)
    expect(streak.streakDays).toBeGreaterThan(0)
    expect(srs.getCard('stub', 'stub/intro/hello-world/fc-1')).not.toBeNull()
  })

  it('completeLesson (second time) does not re-award', () => {
    const award = useAwardActivity()
    const xp = useXPStore()
    award.completeLesson('stub', 'stub/intro/hello-world')
  ...
  it('completeChallenge firstTry awards CHALLENGE_FIRST_TRY', () => {
    const award = useAwardActivity()
    const xp = useXPStore()
    const result = award.completeChallenge('stub', 'stub/intro/hello-world/ch-1', true)
    expect(result.awarded).toBe(true)
  ...
  it('completeChallenge not-firstTry awards CHALLENGE_COMPLETE', () => {
    const award = useAwardActivity()
    const xp = useXPStore()
    const result = award.completeChallenge('stub', 'stub/intro/hello-world/ch-1', false)
    expect(result.awarded).toBe(true)
  ...
  it('completeChallenge replay does not re-award', () => {
    const award = useAwardActivity()
    const xp = useXPStore()
    award.completeChallenge('stub', 'stub/intro/hello-world/ch-1', true)
    const xpAfterFirst = xp.xp
    const result = award.completeChallenge('stub', 'stub/intro/hello-world/ch-1', false)
    expect(result.awarded).toBe(false)
  ...
  it('completeLesson for unknown lesson returns awarded=false', () => {
    const award = useAwardActivity()
    const xp = useXPStore()
    const result = award.completeLesson('stub', 'stub/nope/missing')
    expect(result.awarded).toBe(false)
    expect(xp.xp).toBe(0)
  })
})
